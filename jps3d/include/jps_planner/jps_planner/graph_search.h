...
public:
   ...
   * @param useJps if true, enable JPS search; else the planner is implementing
   * A*
...
private:
  ...
  /// Determine if (x, y, z) with direction (dx, dy) is traverable for safe
  /// corridors
  bool isTraversable(int new_x, int new_y, int new_z, int dx, int dy, int dz);
