...
int main(int argc, char **argv) {
  ...
  // Set up DMP planner
  IterativeDMPlanner2D dmp(false);
  ...
  dmp.setMap(map_util, start); // Set map util for collision checking, must be
                               // called before planning

  // Run DMP planner
  Timer time_dist(true);
  bool valid_dist = dmp.iterativeComputePath(
      start, goal, jps.getRawPath(), 1); // Compute the path given the jps path
