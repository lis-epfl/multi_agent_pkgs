...
int main(int argc, char **argv) {
  ...
  Timer time_jps(true);
  bool valid_jps = planner_ptr->plan(start, goal, 1,
                                     true); // Plan from start to goal using JPS
  double dt_jps = time_jps.Elapsed().count();
  const auto path_jps =
      planner_ptr->getRawPath(); // Get the planned raw path from JPS
  ...
  bool valid_astar = planner_ptr->plan(
      start, goal, 1, false); // Plan from start to goal using A*
  double dt_astar = time_astar.Elapsed().count();
  const auto path_astar =
      planner_ptr->getRawPath(); // Get the planned raw path from A*
  printf("JPS Planner takes: %f ms\n", dt_jps);
  printf("AStar Planner takes: %f ms\n", dt_astar);
  printf("AStar Path Distance: %f\n", total_distance2f(path_astar));

  for (auto pt : path_astar) {
    std::cout << pt.transpose() << std::endl;
  }
  ...
  if (valid_astar) {
    ...
    mapper.map(
        line,
        "opacity:0.4;fill:none;stroke:rgb(1,212,0);stroke-width:5"); // Green
  }

  // Write title at the lower right corner on canvas
  mapper.text(point_2d(origin_x + range_x - 6, origin_y + 1.8),
              "test_planner_2d", "fill-opacity:1.0;fill:rgb(10,10,250);");

  mapper.text(point_2d(origin_x + range_x - 8, origin_y + 1.2),
              "Green: ", "fill-opacity:1.0;fill:rgb(1,212,0);");
  mapper.text(point_2d(origin_x + range_x - 5.5, origin_y + 1.2), "astar path",
              "fill-opacity:1.0;fill:rgb(0,0,0);");

  mapper.text(point_2d(origin_x + range_x - 8, origin_y + 0.6),
